...
# class Cluster:
#     def __init__(self, root, parent_graph,parity = 0):
#         # self.vertices = vertices 
#         self.root = root 
#         self.support = { n: 0 for n in parent_graph.adj[root]  if n != root }  # support table: 0 is ungrown, 1 is half-grown , 2 is fully grown  (mod 3) 
...
#             # new_boundary_nodes = [ n for n in self.parent_graph.neighbors(e) if n not in self.graph.nodes ] 
#             # boundary.extend(  new_boundary_nodes  )

#             # self.vertices.extend( new_boundary_nodes  )
            
#             # self.graph.add_edges_from( [(e, n) for n in new_boundary_nodes] )
#             # self.size += len(new_boundary_nodes )

#         # self.boundary = boundary 
#         # new_support = [] 
#         # for b in boundary:
#         #     new_support.extend( [ n for n in self.parent_graph.neighbors(b) if n not in self.support ]  )

#         # self.support = new_support 

#     def union(self, cluster, fusion_edge = None ):

#         ## update support
#         intersection = {fusion_edge} if fusion_edge else (set(self.support) & set( cluster.support))
#         element = intersection.pop() 

#         self.support[ element ] = (self.support[ element ] + 1)%2
#         self.support.pop( element  )
#         cluster.support.pop(element  ) 
#         self.support.update(cluster.support )


#         ## update boundary
#         combined_boundary = set(self.boundary + cluster.boundary)
#         self.boundary = list( combined_boundary - { n for n in self.graph.neighbors( element  ) if n in self.graph  }  ) 

#         ## update graph, size, parity 
#         self.update( cluster.graph )
#         self.graph.add_edge( self.root, cluster.root )
#         self.children[cluster.root] = cluster 
#         self.size += cluster.size 
#         self.parity = (self.parity + cluster.parity)%2
...
def find_root(tree, node):
    ...
    # tree.add_edges_from([(current, n)] for n in path    )
...
# def generate_spanning_tree(seed,graph, nodes_traversed=[]):
#     if not nodes_traversed:
#         nodes_traversed = dict(zip(  graph.nodes, [False]*len(graph.nodes) ))

#     spanning_tree = nx.DiGraph()
#     spanning_tree.add_node( seed ) 

#     # nodes_traversed = dict(zip(  graph.nodes, [False]*len(graph.nodes) ))
#     nodes_traversed[seed] = True 

#     for node in graph.neighbors(seed):
#         if not nodes_traversed[node]:

#             spanning_tree.add_edge(  seed, node )
#             descendants = [node] + [  n for n in nx.descendants(graph,node) if nodes_traversed[n] == False ]
#             spanning_tree.update( generate_spanning_tree( node, graph.subgraph( descendants ), nodes_traversed  ) )
...
#         # odd_roots = marked_nodes
        
#         Clusters = {node :  {'graph': nx.DiGraph(), 'parity' : 1, 'size' : 0, 'boundary': [], 'support' : [n for n in tanner_graph.adj[node] if n != node], 'children': [] , 'active':True, 'merged': False  }  for node in marked_nodes}
#         Clusters = { node : Cluster( node , tanner_graph, parity = int(node in marked_nodes) ) for node in root_nodes }  
#         active_odd_cluters = [ cluster for cluster in Clusters if cluster.parity == 1 and cluster.active  ]
#         support_table = {key: { 'nodes': [], 'state': 0 }  for key in tanner_graph.nodes if type(key) == int }
#         Cluster_forest = nx.DiGraph()
#         Cluster_forest.add_nodes_from( marked_nodes )
        
#         erasure_edges = []        
#         update_boundary = {key: [] for key in Clusters }
#         root_nodes = [ n for n in Clusters if Clusters[n]['parity'] == 1  and Clusters[n]['active'] == True ]
#         active_root_nodes = [ n for n in Clusters if Clusters[n]['active'] == True ]
...
#                 if cluster.parity == 1 and cluster.active:
#                     cluster.grow()                
#                 for v in cluster.support:
#                     support_table[v].append(cluster.root)

#                 Cluster_forest.update(cluster.graph)


#                 # ##growing cluster to include boundary nodes
#                 # for v in Clusters[node]['boundary']:
#                 #     Clusters[node]['graph'].add_edge( node, v )
#                 #     # Cluster_forest.add_edge(node,v) 
#                 #     # Clusters[node]['size'] += 1

#                 #     if type(v) == str:
#                 #         Cluster_forest.add_edge(node,v)
#                 #         prev_edge_nodes = list( n for n in tanner_graph.neighbors(v) if support_table[n]['state'] == 1 and support_table[n]['nodes'][0][0] in Clusters[node]['graph'].nodes   ) 
#                 #         erasure_edges.extend( prev_edge_nodes )
#                 #         Clusters[node]['size'] += 1
                    
#                 #     if type(v) == int:
#                 #         if node not in support_table[v]['nodes']:
#                 #             support_table[v]['nodes'].append( (node, v) )
#                 #             support_table[v]['state'] = (support_table[v]['state'] + 1)%3


#                 #     update_boundary[node].extend( [ n for n in tanner_graph.adj[v] if n not in Clusters[node]['graph'].nodes ]  )
                    
#                     # if v in virtual_checks:
#                     #     Clusters[node]['active'] = False
#                     #     break  
                                
#                 # Clusters[node]['boundary'] = [n for n in update_boundary[node]   if n not in Clusters[node]['graph'].nodes  ] 
#             fusion_table = { key: support_table[key]['nodes'] for key in support_table if len(support_table[key]['nodes']) == 2 }

#             ### fusion phase


#             for key in list(fusion_table.keys()):
#                 root_1 = find_root(Cluster_forest, fusion_table[key][0] ) 
#                 root_2 = find_root(Cluster_forest, fusion_table[key][1] )

#                 if root_1 != root_2:
#                     ordered = sorted( [ ( Clusters[root_1].size, root_1),  ( Clusters[root_2].size, root_2) ] )[::-1]
#                     ## root should never be a virtual node
#                     Clusters[ordered[0][0] ].union( Clusters[ordered[1][0]] , key )
#                     Cluster_forest.add_edge(ordered[0][0] , ordered[1][0] )
#                 del fusion_table[key]
